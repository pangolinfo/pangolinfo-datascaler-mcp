# DataScaler VOC MCP v0.2.0 — 阿里云 ACK 部署手册
...
scripts\window\deploy-mcp.cmd 0.2.0
```

脚本会:切到根目录 → 通过 WSL 调 `docker-mcp.sh` → 在 WSL 内 `docker build`(Dockerfile 多阶段自己跑 npm ci + npm run build)→ `docker login` ACR → `docker push :0.2.0` 和 `:latest`。
...
```
[pangolinfo-datascaler-mcp] locale=en version=0.2.0
...
# 期望: {"status":"ok","version":"0.2.0","toolCount":25}
