...
【默认首选:免费报告套件 get_social_voc_report_kit】出 VOC 报告**默认走 get_social_voc_report_kit** —— DataScaler 官方一次返回可装配的报告套件(reportSpec 模块清单 + 各模块结构化数据与叙述 + 自包含 CSS + 装配提示),**免费**(不扣积分、不占 AI 额度)。拿到套件后**必须**按其 delivery.instruction + assemblyHints.systemPromptFragment + style(cssSnippet/tokens/classMap)+ modules 装配成一份**完整 HTML 文档**交给用户,**禁止**只用 Markdown 表格充当完整报告。modules 按 id 查找(overview/trends/voice/platforms),schemaVersion 用 startsWith('social-voc-report-kit.v1') 判断;某模块 degraded/failed 时降级渲染已有 data。前置:品牌需已采集完成;若 data not ready / refresh in progress,先 refresh + get_refresh_progress 等完成。免费能力,不要引导用户为报告本身充值。
【进阶付费选项:analyze_brand】只有当用户明确想要**额外的自由提问式深度策略结论**(如"帮我做 KOC 选号""这波公关该怎么回应")、且报告套件满足不了时,才提 analyze_brand。**⚠️ analyze_brand 扣费 600 积分,调它前必须先向用户说明"这会额外花约 600 积分调官方 AI 做一次深度分析"并得到明确同意,不要擅自扣费**。它不是出报告的默认路径,report kit 已能出成品报告。
【用免费只读工具补强报告】装配好报告套件后,可以(推荐)再用下面这些**免费只读**工具补强,让报告更可追溯、更有说服力:补真实用户原话引文、补分平台明细、补 72h 高风险评论样本、补图表。免费工具不扣费,能补的都补上,并在报告里标明每块补充数据来自哪个工具:
...
【兜底:纯自产报告】若 get_social_voc_report_kit 不可用(如上游暂未部署/报错),或用户另有超出套件范围的定制需求,别就此不做 —— 改用上面这些**免费只读**工具自己拉数据、自行撰写一份报告(指标+真实引文+叙事),同样能出可用的报告,只是叙事和结构由你组织。这条兜底路径完全免费。get_voice_share 返回的是**每日 trend 数组**(每天的帖子/提及/声量份额/情感)——别只看静态快照,用它呈现声量与情感随时间的变化,并做简单环比(如"上市/首周 vs 近一周"),趋势比单点更有洞察。
...
[Default first choice: the free report kit get_social_voc_report_kit] By default, produce a VOC report via get_social_voc_report_kit — DataScaler returns an assemble-ready report kit in one call (reportSpec module list + per-module structured data & narrative + self-contained CSS + assembly hints), and it's **free** (no points, no AI quota). After getting the kit you MUST assemble it into a COMPLETE HTML document for the user, following its delivery.instruction + assemblyHints.systemPromptFragment + style (cssSnippet/tokens/classMap) + modules; do NOT pass off Markdown tables as the full report. Look up modules by id (overview/trends/voice/platforms), gate on schemaVersion via startsWith('social-voc-report-kit.v1'); render a degraded/failed module's existing data degraded. Precondition: brand must have collected data; if data not ready / refresh in progress, refresh + get_refresh_progress until done first. It's free — do not push the user to top up for the report itself.
[Advanced paid option: analyze_brand] Only when the user explicitly wants an **extra free-form deep strategy conclusion** (e.g. "pick KOCs for me", "how should we respond to this PR situation") that the report kit can't satisfy, offer analyze_brand. **⚠️ analyze_brand costs 600 points; before calling it you MUST tell the user "this will spend an extra ~600 points to have the official AI run a deep analysis" and get explicit agreement — do not spend on the user's behalf.** It is NOT the default path for producing a report; the report kit already yields a finished report.
...
[Enrich the report with free read-only tools] After assembling the report kit, you can (recommended) enrich it with the free read-only tools below to make it more traceable and convincing: add real user quotes, per-platform detail, 72h high-risk comment samples, charts. Free tools don't charge — add what you can, and label which tool each added block came from:
