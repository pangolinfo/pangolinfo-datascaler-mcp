...
关键:采集页数由用户指定(1-10 页,默认 10)。depthOptions 是 DataScaler 按几档典型页数给出的预估(每档带 estimatedPoints = 采集要花的积分预估),参考它选页数;最终按 create_space 传入的实际 pages 计费。approxPostsPerPlatform(≈关键词数×页数×35%)只是粗估、非保证,实际以采集结果为准。
关键词必须英文:知识空间的社媒发现关键词请用英文(数据源主要索引英文内容,含中日韩字符的词会被上游丢弃)。中文品牌/话题也应译成英文关键词(如"无线耳机降噪"→"noise cancelling earbuds")。不传 extraKeywords 时,DataScaler 默认生成约 12 个英文关键词(品牌词+品类词,随品牌特征浮动,上限 30)。
