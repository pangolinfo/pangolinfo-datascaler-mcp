...
关键:depthOptions 三档 = quick(3页,~20-30min) / standard(5页,~30-60min) / full(10页,~1-1.5h,默认),每档带 estimatedCredits(采集要花的积分预估)。把它翻译成价格给用户确认。
下一步:用户选定**行业(必选,取自 industryCandidates)** + 渠道 + 深度后,调 create_space。
Use when: 用户想了解某品牌/话题的社媒讨论、口碑、声量 —— 这类需求默认从这里开始。
Don't use: 用户明确要长期精细监测(竞品对比/官网/定时) → 用 setup_brand(完整品牌);品牌/空间已存在 → 用 list_brands 找。`,
...
depthOptions: quick(3 pages,~20-30min) / standard(5 pages,~30-60min) / full(10 pages,~1-1.5h, default), each with estimatedCredits. Translate to a price and confirm with the user.
Next: after the user picks an **industry (required, from industryCandidates)** + platforms + depth, call create_space.
...
Don't use: user explicitly wants long-term fine-grained monitoring (competitors/website/schedule) → use setup_brand; brand/space already exists → use list_brands.`,
  }),
  inputSchema,
